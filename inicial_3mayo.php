...
$conteoRojo = 0;
$sqlRojo = "SELECT COUNT(*) AS conteoRojo
FROM captura
WHERE MONTH(fecha_ven) = MONTH(CURDATE())
AND YEAR(fecha_ven) = YEAR(CURDATE())
AND (
    CURDATE() > fecha_ven 
    OR (CURDATE() = fecha_ven AND DAY(fecha_ven) = DAY(CURDATE()))
)
...
$conteoRojoS = 0;
$sqlRojoS = "SELECT COUNT(*) AS conteoRojo
FROM captura
WHERE MONTH(fecha_ses) = MONTH(CURDATE())
AND YEAR(fecha_ses) = YEAR(CURDATE())
AND (
    CURDATE() > fecha_ses 
    OR (CURDATE() = fecha_ses AND DAY(fecha_ses) = DAY(CURDATE()))
)
